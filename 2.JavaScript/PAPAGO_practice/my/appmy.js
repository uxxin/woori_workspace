...
sourceTextArea.addEventListener('input', (event) => {
    ...
    timer = setTimeout(async()=> {
        ...
        console.log(text);
        const detectedResult = await detectLanguage('/dect', text);

        targetLang = changeLanguage(detectedResult, targetLang);

        const { detectLanguage, targetLanguage, translatedText } = await translateLangauge('translate', detectedResult, targetLang, text);

        sourceSelect.value = detectLanguage;
...
//응답에서 국가 코드, 입력값 받아와서 번역 API에 전달 
