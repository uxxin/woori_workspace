...

{
    ...
    void testSave(){
        transaction.begin();
            ...
            lee.setMajor(cs); //연관관게를 설정하는 부분 student -> major
    ...
    void testNotEnoughFK(){
        transaction.begin();
            ...
            // 연관관계의 주인이 아닌 쪽(외래키를 가지지 않은 쪽)에
            // 연관관계(외래키)를 설정하였을 경우
    ...
    void test_Enough_FK_Mapping_In_OOP_Programming() {
        ...
        System.out.println("yoo의 학과 = " + yoo.getMajor()); // CS
        System.out.println("Kang 학과 = " + kang.getMajor()); // CS
    ...
    @Test
    @DisplayName("(해결) 순환 참조 문제 없는지 확인")
    void circularReferenceSolved() {

        // DB에서 조회 자체는 Major 엔티티를 조회
        Major cs = manager.find(Major.class, 1);

        // Entity 타입인 Major를 DTO 타입인 MajorData로 변환
        MajorData csData = MajorData.from(cs); // from(엔티티 객체)
        // -> "Major엔티티로부터 MajorData를 생성(변환)"

        // DTO를 통해 컴공과에 속한 모든 학생 목록 조회
